---
title: "Backfill Import — user-uploaded WhatsApp backup (crypt15)"
...
live-data backfill ([`resources.md`](/docs/architecture/resources) / `AdminService`) can pull contacts and group
...
the API process decrypts it and upserts supported history into the session's tables.
The WhatsApp gateway engine does not handle backup files.
...
Both routes are in the **sessions / `manage`** group ([`http-foundation.md`](/docs/architecture/http-foundation));
...
## Quota & concurrency (`backfill_imports`, [`store.md`](/docs/architecture/store))
...
- Upload cap 256 MiB (`maxBackupUpload` in the HTTP handler); decryption holds the file in memory, the parse streams.
- Import work runs in an API-process goroutine, not a durable task queue. A process
  restart interrupts work; the stale-running rule allows a later retry. The running
  check and insert are separate operations, so this is not a transactional concurrency
  lock across simultaneous requests or API replicas.
