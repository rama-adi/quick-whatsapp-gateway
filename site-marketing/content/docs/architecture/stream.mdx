---
title: "Stream (event fan-out + transport-agnostic Pump)"
...
rules** are shared and documented in [`eventing.md`](/docs/architecture/eventing); this file covers
...
ticket-redeemed — see [`router.md`](/docs/architecture/router)). The former gateway NDJSON
...
an `Authorization` header, so realtime is terminated once on the router. The gateway appends events to its local SQLite journal and sends them over the private
control stream. The API ingests them durably, projects them, publishes Redis events,
and serves client WebSockets. Gateways have no Redis connection.
...
1. **Publisher** — the event consumer that marshals a committed `domain.Event` to
   ...
   Redis channel. The API's committed-event dispatcher invokes it after durable
   ingestion and successful projection. Gateway callbacks never publish directly.
...
and reopens the WebSocket; the router's Pump replays from `event_log` then tails, so retained events can be recovered. Replay is bounded by the implementation's
`resumeReplayLimit`; this is not an unlimited catch-up guarantee. A control-bus drop forces the same reconnect, which
re-validates and fails closed if access is gone.

## Decisions

- **Fan-out only in the Publisher.** Durable `event_log` persistence (for `since`) is
  owned by API ingestion; the Publisher runs only after that transaction commits.
...
- **One transport.** Realtime is WebSocket-only on the router; the API publishes and
  pumps, while the gateway journals. The `Pump`/`Sink` seam keeps the loop transport-agnostic should
  another transport ever be added.

## How it's tested

`go -C backend test ./internal/stream/...`, miniredis for real pub/sub
