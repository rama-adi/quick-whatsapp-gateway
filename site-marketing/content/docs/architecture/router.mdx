...
An engine route remains available while a new desired-state revision is in
flight. The API selects the active, connected gateway and supplies the current
session assignment epoch; the gateway checks that epoch and its local lease
before each live operation. A pending revision for one session therefore does
not hide unrelated sessions. Session creation alone waits for the new revision
to be acknowledged before preparing its device, so an immediate pairing call
can use the newly applied assignment.
...
Newly created gateways have no advertised capabilities until enrollment; registry reads accept that nullable value so pending gateways remain inspectable and listable.
