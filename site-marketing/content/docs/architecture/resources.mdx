...
(currently unavailable through the service's unwired status port). `image` and
other status media return `501 not_implemented`. Ordinary message media sends are
