---
title: "Resource handlers (chats · groups · channels · status · presence · admin)"
description: "Internal architecture and implementation reference."
---

Status: resource services are composed on the API; live operations use private engine RPCs.
...
with gateway registry details available through the admin gateway resource — so
...
**The API resolves live operations to private engine RPCs.** The API composition root sets a resolved facade on each
...
`gateway.ApplicationGatewayAdapter` behind its assignment fence:
...
- **Mutations** (block/unblock, group create/settings/participants/leave) run as
  ...
  sends. The API facade mints a command id for each group mutation; a repeated
  `command_id` returns the stored terminal outcome instead of
...
`backend/cmd/api` wires the API-side facades. Gateway composition owns `wa.LiveOps`;
test constructors can still supply narrow in-process ports. Missing live facades
produce the service's unavailable/not-implemented error.
...
| DELETE | `/chats/{cid}` | transactionally remove the local chat, its messages, and poll/vote rows; WhatsApp-side history is unaffected |
...
`whatsapp_groups` is global metadata. A successful create records the creating
session's own LID as the group's creator membership so it appears in that
session's list immediately. Mutations + invite links go through
`GroupOps`:
...
`POST /status` discriminates on `type`. Text stories send to
`status@broadcast` through the API-owned outbound scheduler and return the
acknowledged WhatsApp message ID; a queued or missing acknowledgement is not
reported as success. `image` and other status media return
`501 not_implemented`. Ordinary message media sends are
implemented by the outbound pipeline.
