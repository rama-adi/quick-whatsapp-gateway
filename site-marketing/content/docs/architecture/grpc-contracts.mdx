---
title: "gRPC contract tooling and compatibility"
...
- the module rooted at `backend/proto/public` defines the API-facing gRPC surface under `v1`. Only
...
`backend/internal/application` defines consumer-owned engine capabilities.
`backend/internal/wa.ApplicationGatewayAdapter` supplies session state, presence, read receipts,
lifecycle, messaging, and resource operations from the gateway runtime. The gateway composition
root constructs this adapter and serves it through private engine RPCs; API services call the
remote facade for live work.

Commands and results carry organization, session, and gateway identifiers. Mutations additionally
carry a stable `command_id` and `assignment_epoch`. Engine admission validates the current
connection and assignment; durable mutations use the gateway command-result ledger for replay.
The RPC operation table below identifies ledger-backed and idempotent operations. Context carries deadlines, cancellation, and tracing rather than
...
| `DownloadMedia` | slow read | downloads a captured WhatsApp attachment through its owning session; validates organization and assignment epoch, and applies the existing 16 MiB media policy |
...
terminally for that process: an initial DRAIN prevents session startup, while a DRAIN after startup first
...
supervisor keeps reconnecting. A later RUN Welcome starts assigned sessions, reports READY, and opens
...
error. The same lifecycle watcher is installed for an immediate or delayed startup, so a DRAIN arriving
...
A post-Welcome RUN starts or affirms engine work
...
terminal report. Certificate renewal, desired-state assignment reconciliation, and engine commands are wired
into the runtime. Terminal authorization failures terminate the gateway; transient transport
failures reconnect while admission remains governed by acknowledged leases. Disconnect clears liveness but does not invent a terminal
...
`backend/` after this move, or the WhatsApp Gateway checkout directory for commits before the Go module moved into `backend/`.
