...
Reply choices require nonblank IDs/titles; reply IDs are unique within the
message. Button messages allow one to three actions, each with a label of at
most 20 Unicode code points. An omitted `kind` means `reply`; `kind: url`
requires an HTTPS `url`, and `kind: copy` requires a nonempty `code`. URL and
copy-code values can each contain at most 32,768 UTF-8 bytes, the largest
payload verified in live WhatsApp tests. An optional `headerImage` uses the
normal media source (`data` or `url`) and
upload path. The HTTP schema and gateway engine enforce the button limits
before sending. Menus require a title, sections, and rows. The existing HTTP
request-body bound still applies.

The adapter builds native-flow `quick_reply`, `cta_url`, and `cta_copy` buttons
and adds the required `biz` metadata. When a footer exists, the interactive
builder adds a newline to the body for visual separation. Image headers upload under the image
media key and populate `InteractiveMessage.Header`. Lists use legacy
`ListMessage`, for which whatsmeow adds list metadata.
...
- `SendResult` — `{Mode, WAMessageID, ReservedMessageIDs, Status, Timestamp, OutboxID, Replayed}`.

The API returns `reservedMessageIds` for a durably accepted send, including async,
pending, and idempotent replay results. These are the native IDs reserved by the
gateway command: a single ID for ordinary messages, or the album container followed
by all child IDs in media order. The scheduler and actual WhatsApp sender share the
same allocator. Clients can attach application context to these IDs before delivery
settles, including when an album has only partially arrived. Reservations are not
delivery receipts: only the existing `mode`, `status`, and `waMessageId` fields
establish a committed outcome. Operation requests such as reactions have no send
reservations. The mapping survives payload media scrubbing and process restarts;
clients must never duplicate the allocator or infer IDs from an outbox identifier.
...
- **API-owned scheduling (gRPC Increment 6)**: the API's `service.OutboundScheduler` owns
  ...
  ambiguous. The gateway derives each native WhatsApp message ID from the durable command ID
  and the message's ordinal within that command, so replay uses the same native ID across
  process restarts. The ledger does not promise exactly-once delivery across a WhatsApp
  acknowledgement gap.
...
- **Ambiguity policy**: only deterministic pre-dispatch rejections (domain validation errors,
  ...
  backoff (`next_attempt_at`). After the configured attempt budget, the row becomes terminal
  `unknown`: its remote effect may have committed, so an idempotent send replay returns `202`
  with the same outbox ID and never reports a definite failure or dispatches that command again.
  A channel agent keeps later messages behind that unresolved action. The stale-`sending` lease must exceed the engine
...
- **Idempotency is durable via the outbox**: sync sends with a key persist a
  ...
  The reaction endpoint accepts `Idempotency-Key` and uses the same outbox
  lookup before dispatch. Repeating the key returns the row's current state;
  a queued operation progresses through the existing worker, and a sent
  operation returns its original WhatsApp message ID.
...
- **Worker ownership is a database CAS**: before dispatching a queued task, a worker calls
  ...
  deterministic `failed` and unresolved `unknown` outcomes, so settled commands never re-enter the retry loop. Batch recovery uses
