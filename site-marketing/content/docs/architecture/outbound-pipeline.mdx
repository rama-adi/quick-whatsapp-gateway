...
The public schema and reusable outbound adapter accept buttons and lists. Current
`backend/cmd/gateway/engine_dispatcher.go` has no `buttons` or `list` dispatch branch;
production engine execution returns `unsupported send type`. The request shapes
and adapter behavior below describe implemented components awaiting that wiring.
Incoming selection normalization remains implemented independently.
...
Production `OutboundScheduler.recordDispatchedSend` calls the API-side
`MessageRecorderAdapter` after a successful send. It persists outbound history and
poll metadata. The reusable `Sender` offers an equivalent recorder hook described
below, but the gateway executor does not write the shared database.
...
- text → `Conversation`, or `ExtendedTextMessage{ContextInfo}` when a reply or
  mentions are present. the reusable quote resolver can enrich `replyTo` from stored history. Current
  gateway `engineDispatcher.Dispatch` constructs `QuoteInfo{ID: req.ReplyTo}`, so `replyTo`
  carries a stanza-only quote unless quote data is otherwise supplied to the adapter.
