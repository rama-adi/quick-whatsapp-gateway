---
title: "Store — MySQL app-data repositories (backend/internal/store)"
...
them read-only via Drizzle ([`frontend.md`](/docs/architecture/frontend) § hybrid reads). Masterplan §6, §7.
...
signup, so solo use is a one-member org. The API authorizes from JWT claims /
api-key `reference_id`, never by joining `member` on the hot path ([`trust-model.md`](/docs/architecture/trust-model)).
...
| `outgoing_message_event_claims` | `EventLogRepo` + `GatewayEventIngestRepo` | composite organization/session/chat/WhatsApp-message identity; first committed event owner |
...
`apikey` and the other better-auth tables are **not** in the WhatsApp application-data repository set — they are frontend-owned
(Drizzle). `APIKeyRepo` here is **read-only** (`GetByHash`) and used solely by `backend/internal/authz`
to verify keys ([`api-keys.md`](/docs/architecture/api-keys)); it is not a key-management repo.

`Store` (`store.go`) aggregates the repos; `New(db *sql.DB)` builds the set. The generated sqlc
package lives under `backend/internal/store/storedb` and is kept behind the `backend/internal/store` package boundary; callers should
...
- **`gateways` + `wa_sessions.gateway_id`** are the session-pinning seam
  ([`whatsmeow-store.md`](/docs/architecture/whatsmeow-store), masterplan §4.5); with the central router (Increment A)
  `gateways` is now the live **routing table** the router reads, and `wa_sessions.gateway_id` (already
  `NOT NULL`) is **authoritative for routing** ([`router.md`](/docs/architecture/router), [`session-manager.md`](/docs/architecture/session-manager)).
...
The retention worker is a global API-maintenance writer, not an
...
- **DM alias resolution.** Messages may be captured under either the contact's
  ...
  discovered. The merge first combines canonical rows, then deletes merged
  aliases, and finally renames remaining aliases with a plain update. This
  avoids MySQL error 1093 from selecting the update target in a correlated
  subquery. Concurrent uniqueness conflicts leave the event retryable.
  Ambiguous or unknown phone JIDs are left unchanged.
...
- **`messages` has two writers, both API-side.** The projection consumers write
  ...
  *other* devices) from committed events; the outbound scheduler writes the gateway's own sends
  ...
  successful dispatch — see [`outbound-pipeline.md`](/docs/architecture/outbound-pipeline). Both go through
  ...
  merges with live capture (see [`backfill-import.md`](/docs/architecture/backfill-import)).
...
- **Concurrency.** `WebhookDeliveryRepo.ClaimDue` selects due rows with `FOR UPDATE SKIP LOCKED`
  ...
  The API's sent-event append and a gateway `message.from_me` echo claim the same
  `outgoing_message_event_claims` row in the event-log transaction. Its composite
  primary key makes ownership serializable across either arrival order; the
  winning API outbox ID or gateway journal event ID remains the public ULID.
  Duplicate gateway journal IDs are acknowledged independently and completed
  without a second event-log row or fan-out. The row follows its session on delete.
  Historical sends predating this table have no claim until a separately verified
  upgrade handles that deployment's existing data.
...
`media_buckets`, `session_media_storage`, and `media_assets` are API-owned MySQL tables for S3 connections, session bindings, and durable uploads/retention cleanup. Credentials and download sources are encrypted. The frontend accesses them through the API. See [attachment storage](/docs/architecture/media-storage).

## Sticker content

Migration `0017_stickers` adds globally content-addressed `sticker_blobs` and
session/chat/message references in `sticker_messages`. The blob primary key is
SHA-256, with no organization key. Bytes are stored once and expanded only at event
delivery boundaries. Session deletion cascades message references; shared content
is retained. See [shared sticker content](/docs/architecture/media-storage#shared-sticker-content)
for capture, quote resolution, and delivery behavior.
