...
9. Any media message (image/video/audio/document/sticker) → media **metadata only**.
...
Inbound media is **not downloaded** by normalization. For media messages `HasMedia=true` and `MediaInfo`
