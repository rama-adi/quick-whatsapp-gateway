...
Status: implemented on the API service with private engine calls for live operations.
...
sub-resources (check / picture / about / block) go through the API service and its
private engine facade. The API owns shared-MySQL writes.
...
**Production routing:** the API dispatches these calls through private engine RPCs. The
...
gateway executes all four through `gateway.ApplicationGatewayAdapter` behind its
assignment fence.
