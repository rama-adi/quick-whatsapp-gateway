---
title: "Sign in with WhatsApp — OAuth 2.1 / OIDC provider (backend/internal/oidp)"
...
the private gateway PKI/control-plane trust, which has a separate identity and certificate lifecycle.
...
| GET | `/.well-known/oauth-jwks.json` | OIDC signing JWKS (`active` + `next`). Dedicated OAuth signing keyset; private gateway trust uses mTLS certificates. |
...
consumed before message-table projection and external event fan-out. Their durable ingest
envelope is committed before this consumer runs.
...
on that session is consumed by the first committed-event consumer — consumed before message-table projection and external fan-out
to webhooks or the event stream — *even when the code is wrong or expired*. This is deliberate: login traffic (and
...
The API owns the Lua claim, pending Redis state, and durable OAuth records. It sends
verification feedback through its outbound scheduler and the assigned gateway's private engine
RPCs. Verified pending state survives an API process restart in Redis; grants, auth codes,
and tokens are issued by API protocol handlers. The atomic Redis transition gives every duplicate observation one authoritative claim result.
...
### 6.2 Dashboard — `web/app/routes/_app/user/oauth-apps*`
...
## 10. Maintaining this contract

Management CRUD is defined by Go Huma operations and shared DTOs; regenerate the OpenAPI,
frontend types, and reference pages with `make gen`. Protocol endpoints follow OAuth/OIDC
wire formats and are mounted by `backend/internal/oidp/provider.go`. Update this page and
the integration guide when their behavior changes.

## 11. Design decisions

The implementation uses these choices:
