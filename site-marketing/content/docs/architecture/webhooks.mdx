...
(`organization_id`), mutated through the API service's `/webhooks` routes
(`RequireManage`, §13), and surfaced read-only in the frontend dashboard (§6.2). The
API service owns shared-MySQL writes; gateways never dispatch webhooks.
...
- **Enqueue** (`Enqueuer`): the webhook consumer called by event fan-out per
  ...
  API durable ingestion reaches it through `service.CommittedEventWorker` and
  `service.CommittedEventDispatcher` after commit and projection.
- **Dispatch** (`Dispatcher`): claims due deliveries and sends them. Exposes
  ...
  by the API's five-second ticker (`startWebhookDispatchLoop` in `backend/cmd/api/main.go`).
...
All collaborators are **consumer interfaces** defined in `webhooks.go`; API composition
...
- **Matching.** `events: ["*"]` matches everything; otherwise an exact element
  match. Empty list matches nothing. The webhook MySQL repository filters in SQL; the dispatcher
...
- **Per-webhook isolation.** In `Enqueue`, a single webhook's dedup-check or
  create failure is logged while fan-out continues to the other hooks. Errors
  are aggregated and returned so the durable event remains retryable. Existing
  delivery identities prevent retries from duplicating successful inserts.
...
- **Enqueue** — creates pending rows for matches (with correct fields and
  ...
  per-webhook `Create` failure keeps the event retryable. The isolated API E2E
  suite injects a MySQL delivery-insert failure after event persistence and
  checks recovery with one event, one delivery, and one WhatsApp send.
...
- A `WebhookRepo` + `WebhookDeliveryRepo` over MySQL (`store` package). `ClaimDue`
  is required to be atomic (`SELECT … FOR UPDATE SKIP LOCKED` or a CAS claim column) so
...
- The API ticker calls `Dispatcher.DeliverDue` every five seconds with
  `webhooks.DefaultClaimLimit`. Its HTTP client timeout is 30 seconds, both values
  defined in `backend/cmd/api/main.go`. Committed-event fan-out invokes
  `Enqueuer.Enqueue`; this path does not enqueue an Asynq job.
