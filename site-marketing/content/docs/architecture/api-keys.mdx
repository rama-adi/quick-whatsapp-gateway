...
plugin**; the API **verifies** presented keys against the shared `apikey` table. The v1
custom Go keys (argon2id, `wak_` prefix, `backend/internal/crypto` + an `api_keys` table) are gone.
Masterplan §4.2. The trust seam as a whole is documented in [`trust-model.md`](/docs/architecture/trust-model).
...
`backend/internal/authz` validates a presented key locally — no callback to the frontend — so the API
can verify API keys if the frontend is down. `APIKeyVerifier.VerifyKey`
...
Validated keys sit in a per-API-process positive cache (`backend/internal/authz/apikey_cache.go`, ~60 s TTL,
fail-closed). Revocation is **instant** via the Redis control bus: the frontend publishes
`ctrl:apikey.revoked {keyId}` (or `ctrl:user.banned`) on revoke/ban; API processes evict the cache
entry and drop live WebSocket event streams the key authenticated. The TTL is the backstop for a missed
message; a restarted API starts with an empty positive cache. Full detail: [`trust-model.md`](/docs/architecture/trust-model) §
control bus.
...
test** creates a key in better-auth and validates it in the API verifier, locking the assumption.
