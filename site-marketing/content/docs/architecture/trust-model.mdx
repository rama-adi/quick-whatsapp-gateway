---
title: "Trust & auth model (backend/internal/authz + backend/internal/controlbus)"
...
> non-retryable state conflict mapped to a public HTTP conflict response. The private enrollment transport invokes this service.
...
gateway NDJSON is removed ([`router.md`](/docs/architecture/router)).
...
live in [`grpc-contracts.md`](/docs/architecture/grpc-contracts) and [`router.md`](/docs/architecture/router).
...
## Restart and assignment reconciliation

Two restart safeguards apply:

- API-owned assignment snapshots authorize gateway session startup. The desired-state reconciler
  stops unassigned or expired sessions and requires the expected local keystore device. Session
  and organization ownership stay in the API; gateway startup uses only fenced assignments and SQLite.
...
Fenced assignments protect WhatsApp session ownership. API DB lookup plus cache TTL
...
> private TLS 1.3 listener consumes this identity when its address is configured.
...
prevents session startup; DRAIN after startup drains already-admitted requests before shutting down manager
...
receives a failure report and never restarts work. Certificate renewal scheduling and rollover, assignment reconciliation, and terminal
authorization handling are wired into the runtime described above.

For an installed identity, a transient pre-Welcome outage brings up diagnostics unready with engine
admission closed and starts assigned sessions only after a later RUN Welcome. The lifetime lifecycle
watcher also covers that delayed start. Terminal authentication and protocol failures, whether
