...
Production wiring lives in `backend/cmd/gateway`: `wa.Manager` forwards raw
whatsmeow events to `gateway/waadapter.InboundPipelineHandler`. The gateway uses
`inbound.NoopRepos`, a journal-only event sink, a no-op webhook sink, and manager-backed
WhatsApp live operations. It has no MySQL or Redis connection. Automatic read receipts
and composing presence happen locally before the normalized event enters the SQLite
journal. `InboundEventTimeout` is explicitly zero in this composition; the former
10-second callback deadline does not apply.

The API durably ingests journal events from the private control stream. Its
`CommittedEventWorker` runs OAuth login interception, then `EventProjectionConsumer`
(store writes), then Redis publication and webhook enqueue. Realtime clients use
WebSockets, not NDJSON. A failed projection leaves durable work incomplete for retry
and prevents later events for that session from overtaking it.

The stage and port details below describe the reusable pipeline. Capture/persist
ports are deliberately no-ops on the gateway; equivalent database projections run
on the API after ingest. The optional pipeline OAuth interceptor is not installed
on the gateway. Production OAuth interception therefore suppresses ordinary chat
projection and live/webhook fan-out **after** the credential-bearing envelope has
already reached the local journal and API event log; it is not a promise that those
durable transport stores never contain login commands.
...
small consumer interface supplied at composition:
...
- `Normalizer.Normalize(ctx, evt any, sessionID, organizationID string) (domain.Event, *NormalizedMessage, bool)`
  ...
  the adapter can use a whatsmeow client and a poll-options reader. Current gateway
  wiring passes a nil poll reader, so vote decryption/option resolution is skipped.
...
- **Decoupling.** `NormalizedMessage` is defined here, not in `events`. The
  Normalizer maps whatsmeow protobufs onto it; the pipeline operates
