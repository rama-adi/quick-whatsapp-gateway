...
func classify(e *events.Message, msg *waE2E.Message, nm *NormalizedMessage) {
	switch {
	case fillInteractiveReply(msg, nm):
...
func catalogForSubtype(st MessageSubtype, fromMe bool) (string, PersistKind) {
	switch st {
	case SubtypeInteractiveReply:
		return domain.EventMessageInteractiveReply, PersistMessage
...
func messagePayload(nm *NormalizedMessage) MessagePayload {
	return MessagePayload{
		InteractiveReply: nm.InteractiveReply,
...
func fillInteractiveReply(msg *waE2E.Message, nm *NormalizedMessage) bool {
	reply := &InteractiveReplyData{}
	var info *waE2E.ContextInfo
	switch {
	case msg.GetButtonsResponseMessage() != nil:
		m := msg.GetButtonsResponseMessage()
		reply.Kind, reply.ID, reply.Title = "button", m.GetSelectedButtonID(), m.GetSelectedDisplayText()
		info = m.GetContextInfo()
	case msg.GetListResponseMessage() != nil:
		m := msg.GetListResponseMessage()
		reply.Kind, reply.ID, reply.Title = "list", m.GetSingleSelectReply().GetSelectedRowID(), m.GetTitle()
		info = m.GetContextInfo()
	case msg.GetInteractiveResponseMessage() != nil:
		m := msg.GetInteractiveResponseMessage()
		flow := m.GetNativeFlowResponseMessage()
		switch flow.GetName() {
		case "quick_reply":
			reply.Kind = "button"
		case "single_select":
		...
		if json.Unmarshal([]byte(flow.GetParamsJSON()), &params) != nil {
			return false
		}
		reply.ID, reply.Title = params.ID, params.DisplayText
		if reply.Title == "" {
			reply.Title = params.Title
		}
		if reply.Title == "" {
			reply.Title = m.GetBody().GetText()
		}
		info = m.GetContextInfo()
	default:
		return false
	...
	nm.Subtype, nm.MessageType = SubtypeInteractiveReply, "interactive_reply"
	nm.InteractiveReply, nm.Body = reply, reply.Title
	applyContext(info, nm)
	return true
}
