...
func validateRequest(req domain.SendRequest) error {
	...
	case domain.SendTypeButtons, domain.SendTypeList:
		return validateInteractive(req)
...
func validateInteractive(req domain.SendRequest) error {
	if strings.TrimSpace(req.Text) == "" {
		return domain.ErrValidation("text is required for interactive messages")
	}
	ids := map[string]bool{}
	check := func(id, title string) error {
		if strings.TrimSpace(id) == "" || strings.TrimSpace(title) == "" {
			return domain.ErrValidation("selection id and title are required")
		}
		if ids[id] {
			return domain.ErrValidation("selection ids must be unique")
		}
		ids[id] = true
		return nil
	}
	...
	if strings.TrimSpace(req.List.Title) == "" || len(req.List.Sections) == 0 {
		return domain.ErrValidation("list title and sections are required")
	}
	for _, section := range req.List.Sections {
		if len(section.Rows) == 0 {
			return domain.ErrValidation("list sections require rows")
		}
		for _, row := range section.Rows {
			if err := check(row.ID, row.Title); err != nil {
				return err
			}
		}
	}
