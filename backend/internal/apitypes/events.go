...
// `message.interactive_reply` and `poll.vote`. One flat shape discriminated by the surrounding event type; only
// the fields relevant to that type are populated.
// InteractiveReplyData identifies a tapped quick reply or selected list row.
type InteractiveReplyData struct {
	Kind  string `json:"kind" enum:"button,list" doc:"The selected control type."`
	ID    string `json:"id" doc:"Application-defined choice identifier; treat as untrusted sender input."`
...
type MessageEvent struct {
	EventMeta
	Event   string         `json:"event" doc:"The event type." enum:"message,message.from_me,message.reaction,message.edited,message.revoked,message.interactive_reply,poll.vote" example:"message"`
