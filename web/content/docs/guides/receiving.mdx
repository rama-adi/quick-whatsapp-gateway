...
| `message.status` | WhatsApp reports a receipt for message IDs; advance known statuses monotonically and ignore unknown/stale IDs. |
| `message.reaction` | Someone reacts to a message. |
| `message.interactive_reply` | A quick-reply button or list option is selected; includes the selection ID and original message reference when supplied by WhatsApp. |
...
<Callout type="info">
**Attachment storage is configured per session.** Media messages carry `hasMedia: true`. Linked sessions receive a tracked `media.id`; `media.ready` supplies the URL after upload. See [attachment storage](/docs/guides/attachment-storage).
</Callout>
