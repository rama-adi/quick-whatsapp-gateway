...
export const gateways = mysqlTable("gateways", {
	...
	reconciliationStatus: mysqlEnum("reconciliation_status", ['pending','healthy','degraded']).default('pending').notNull(),
	keystorePresent: tinyint("keystore_present"),
	keystoreBytes: bigint("keystore_bytes", { mode: "number" }),
	keystoreIntegrity: mysqlEnum("keystore_integrity", ['healthy','missing','corrupt']),
	keystoreCheckedAt: bigint("keystore_checked_at", { mode: "number" }),
	journalState: mysqlEnum("journal_state", ['healthy','degraded','paused','critical']),
	journalEntries: bigint("journal_entries", { mode: "number", unsigned: true }),
	journalBytes: bigint("journal_bytes", { mode: "number", unsigned: true }),
},
(table) => [
	index("idx_gateways_creator").on(table.createdByUserId),
	index("idx_gateways_reconciliation").on(table.connectionMode, table.reconciliationStatus, table.desiredRevision, table.appliedRevision),
