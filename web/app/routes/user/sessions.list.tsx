// User: my sessions — list + create. Surface agent: user.
//
// Lists the tenant's WhatsApp sessions (cursor-paginated, live status via the
// event stream), with a create dialog ({label,start,autoRead,presenceTyping})
// and quick lifecycle actions. Detail/QR/pairing live on session.overview.
...
import type { CreateSessionRequest, SessionAction, WASession } from "~/lib/api/types";
...
import { formatTimestamp, SessionStatusBadge } from "./_ui";
        ...
        <Card key={s.id}>
          <CardHeader className="flex-row items-start justify-between gap-2 space-y-0">
            <div className="space-y-1">
              <CardTitle className="text-base">
                <Link
                  to={`/user/sessions/${encodeURIComponent(s.id)}`}
          ...
          <CardFooter className="justify-between gap-2">
            <Button asChild size="sm" variant="ghost">
              <Link to={`/user/sessions/${encodeURIComponent(s.id)}`}>
                Open
              </Link>
            </Button>
            <Button
