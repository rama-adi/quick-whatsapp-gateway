// User: session overview — live status, lifecycle actions, QR display, and
// phone pairing-code entry. Surface agent: user.
//
// Live wiring: useSession()/useSessionMe() are kept fresh by the cacheBridge
// (session.status event). useSessionQR() is seeded from GET /qr and refreshed
// live by the auth.qr event; pairing codes arrive via auth.code. When the
// stream degrades to polling we lean on usePollingInterval so status/QR still
// advance. The QR image itself is rendered same-origin via ?format=image so the
// cookie session authenticates the <img> request — no extra QR dependency.
...
import { CopyButton, formatTimestamp, SessionStatusBadge } from "./_ui";
...
export default function SessionOverview() {
  ...
  // When the stream is degraded, fall back to polling the session row so the
  // status badge keeps advancing through starting → scan_qr_code → working.
  useEffect(() => {
  ...
  const needsPairing =
    s.status === "scan_qr_code" || s.status === "starting" || s.status === "stopped";
...
function QrPanel({ sessionId, ready }: { sessionId: string; ready: boolean }) {
  ...
  const poll = usePollingInterval();
  // Re-key the <img> when the code changes (live via auth.qr) or on manual
  // refresh so the browser re-fetches the same-origin PNG with the cookie.
  ...
  const imgSrc = code
    ? `${apiUrl(`/sessions/${encodeURIComponent(sessionId)}/qr`)}?format=image&v=${encodeURIComponent(code)}&n=${nonce}`
    : null;
    ...
    <form onSubmit={submit} className="space-y-4">
      ...
      {pairing.data?.code && (
        <div className="space-y-2 rounded-md border bg-muted/40 p-3">
          <p className="text-xs text-muted-foreground">
            Enter this code on your phone (WhatsApp → Linked devices → Link with
            phone number):
          </p>
