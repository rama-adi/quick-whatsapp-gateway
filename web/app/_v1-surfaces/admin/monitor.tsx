...
// Surface: admin. Reads the shared event bus (events=*, no session filter); it
// does NOT open its own connection — the single EventStreamProvider owns that.

import { useCallback, useMemo, useState } from "react";
import { requireAdmin } from "./_guard";
import { StreamIndicator, fmtTime } from "./_shared";
      ...
      <Card>
        <CardHeader className="gap-3">
          <div className="flex flex-wrap items-center gap-2">
            <Input
              type="search"
              placeholder="Filter by event, session, tenant, id…"
              value={text}
    ...
    <Card>
      ...
      <CardContent className="space-y-3">
        <dl className="grid grid-cols-2 gap-x-4 gap-y-1 text-sm sm:grid-cols-4">
          ...
          <Meta label="tenant" value={event.tenant} mono />
