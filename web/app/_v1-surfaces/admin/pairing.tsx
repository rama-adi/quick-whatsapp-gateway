...
// platform admin session(s). Surface: admin.
...
export default function AdminPairing() {
  ...
  const adminSessions = useMemo<WASession[]>(
    () => (q.data?.pages.flatMap((p) => p.data) ?? []).filter((s) => s.isAdminSession),
    [q.data],
  );
...
function SessionPicker({
  ...
  sessions: WASession[];
  overrides: Record<string, NonNullable<WASession["status"]>>;
...
function PairingPanel({
  ...
  session: WASession;
  liveStatus?: NonNullable<WASession["status"]>;
}) {
