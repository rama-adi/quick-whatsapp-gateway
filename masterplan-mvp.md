...
- **Testing (MANDATORY):** every subsystem ships tests in the **same change** as the code — no package merges without meaningful coverage. Table-driven Go tests for pure logic (domain helpers, config parsing, normalization, idempotency, rate limiting, HMAC, event envelope, error mapping); repos tested against SQLite (or a MySQL test container) rather than mocks where practical; HTTP handlers tested via `httptest` (validation, auth, error envelopes, pagination). External boundaries (whatsmeow client, Authula, Redis) are abstracted behind consumer interfaces and faked in tests. `go test ./...` must pass and is a required gate in CI and before every commit; frontend gets component/route tests (Vitest) for the realtime/data layer.
