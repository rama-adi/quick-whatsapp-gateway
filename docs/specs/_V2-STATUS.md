...
| `oauth.md` | 🚧 in progress | R-OIDC | NEW — "Sign in with WhatsApp": OAuth 2.1 / OIDC provider on the API (`internal/oidp`); DM/group-mention verification via a login interceptor bridged to the gateway's inbound pipeline; two-code pending-auth model in Redis (API-side); consent page + app CRUD in web/. Milestones in [`../../oauth2-progress.md`](../../oauth2-progress.md). **Increment 9 note:** the gateway's OIDP login-interceptor wiring was removed with its Redis dependency; the interceptor bridge needs re-homing on the API side (tracked follow-up). |
