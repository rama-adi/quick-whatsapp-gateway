...
gRPC control-plane migration). Live: API-local REST + public gRPC, auth termination, gateway
...
mTLS control stream, with no MySQL or Redis dependency (see [`trust-model.md`](trust-model.md),
[`http-foundation.md`](http-foundation.md)).
...
3. **Serve API-locally.** Repository reads/writes execute in-process; live operations
   ...
   ([`grpc-contracts.md`](grpc-contracts.md), [`outbound-pipeline.md`](outbound-pipeline.md)).
...
[`queue.md`](queue.md), [`trust-model.md`](trust-model.md)). On `ctrl:apikey.revoked` the API
...
`evt:*` after projections succeed ([`eventing.md`](eventing.md)). The
