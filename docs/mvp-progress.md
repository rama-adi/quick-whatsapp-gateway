...
| **Increments 7–10** — remaining cutover | ⬜ Planned | Migrate remaining live resource operations (contacts, groups, channels, status, sync, pairing, logout, admin ops), implement public gRPC, remove gateway HTTP/MySQL/Redis, then harden the sole target deployment. |
