...
| **R3** — Frontend scaffold | ⬜ Planned | TanStack Start app; better-auth (email/password, twoFactor, admin, apiKey, jwt, **organization**) on MySQL + migrations; `definePayload` → `activeOrganizationId`+role in JWT; **personal-org-on-signup** hook; copy shadcn `components/ui`; port login/register/TOTP/admin/keys; **org switcher**. |
