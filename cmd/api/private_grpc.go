...
func (s mysqlGatewayCredentialStore) AuthorizeGatewayCertificate(
	ctx context.Context,
	identity gatewayIdentity,
	now int64,
) (gatewayIdentity, error) {
	err := s.db.QueryRowContext(
		ctx,
		`SELECT c.id, c.authority_id FROM gateways g JOIN gateway_certificates c ON c.gateway_id=g.id
		...
		AND c.not_before<=? AND c.not_after>? LIMIT 1`,
		identity.GatewayID,
		identity.SerialNumber,
		identity.Fingerprint,
		now,
		now,
	).Scan(&identity.CertificateID, &identity.AuthorityID)
	return identity, err
}
...
func (a privateGatewayAuthenticator) unary(
	ctx context.Context,
	req any,
...
func (a privateGatewayAuthenticator) stream(
	srv any,
...
func strictGatewayIdentity(cert *x509.Certificate) (gatewayIdentity, error) {
	notLeaf := cert == nil || cert.IsCA || !cert.BasicConstraintsValid ||
		cert.KeyUsage != x509.KeyUsageDigitalSignature
	extraIdentities := len(cert.URIs) != 1 ||
		len(cert.DNSNames) != 0 ||
		len(cert.IPAddresses) != 0 ||
		len(cert.EmailAddresses) != 0
	if notLeaf || extraIdentities {
		return gatewayIdentity{}, errors.New("invalid gateway certificate policy")
	}
	wrongExtKeyUsage := len(cert.ExtKeyUsage) != 2 ||
...
func (h gatewayEnrollmentGRPC) Renew(
	ctx context.Context,
	req *gatewayv1.GatewayEnrollmentServiceRenewRequest,
) (*gatewayv1.GatewayEnrollmentServiceRenewResponse, error) {
	...
	input := service.RenewalInput{
		Credential: service.RenewalCredential{
	...
	result, err := h.renewal.Renew(ctx, input)
	if err != nil {
		return nil, renewalStatus(err)
	}
	return &gatewayv1.GatewayEnrollmentServiceRenewResponse{
		GatewayId:           result.GatewayID,
		CertificateChainPem: []byte(result.CertificatePEM),
		TrustBundlePem:      []byte(result.TrustBundlePEM),
		AuthorityId:         result.AuthorityID,
		SerialNumber:        result.SerialNumber,
		NotBeforeUnixMs:     result.NotBefore,
		NotAfterUnixMs:      result.NotAfter,
	}, nil
}

func (h gatewayEnrollmentGRPC) Enroll(
	ctx context.Context,
	req *gatewayv1.GatewayEnrollmentServiceEnrollRequest,
) (*gatewayv1.GatewayEnrollmentServiceEnrollResponse, error) {
	if req == nil {
		return nil, status.Error(codes.InvalidArgument, "invalid enrollment request")
	}
	invalidToken := len(req.Token) == 0 || len(req.Token) > maxEnrollmentTokenBytes
	invalidCSR := len(req.CsrDer) == 0 || len(req.CsrDer) > pki.MaxCSRBytes
	if invalidToken || invalidCSR {
		return nil, status.Error(codes.InvalidArgument, "invalid enrollment request")
	}
	...
	return &gatewayv1.GatewayEnrollmentServiceEnrollResponse{
		GatewayId:           result.GatewayID,
		CertificateChainPem: []byte(result.CertificatePEM),
		TrustBundlePem:      []byte(result.TrustBundlePEM),
		AuthorityId:         result.AuthorityID,
...
func (h privateGatewayHealth) Check(
	context.Context,
	*gatewayv1.GatewayHealthServiceCheckRequest,
) (*gatewayv1.GatewayHealthServiceCheckResponse, error) {
...
func (s gatewayControlStore) Accept(
	ctx context.Context,
	gatewayID string,
	hello apigateway.Hello,
) (apigateway.Connection, error) {
...
func (s gatewayControlStore) Heartbeat(
	ctx context.Context,
	gatewayID string,
	epoch uint64,
	heartbeat apigateway.Heartbeat,
) (apigateway.DesiredLifecycle, error) {
...
func (s gatewayControlStore) DesiredState(
	ctx context.Context,
	gatewayID string,
	epoch, revision uint64,
	leaseExpiresAt time.Time,
) (apigateway.DesiredState, error) {
	connection := domain.GatewayConnection{GatewayID: gatewayID, ConnectionEpoch: epoch}
	assignments, current, err := s.repo.DesiredStateForEpoch(ctx, connection, revision, leaseExpiresAt.UnixMilli())
...
func (s gatewayControlStore) PersistDesiredStateReport(
	ctx context.Context,
	gatewayID string,
	epoch uint64,
	report apigateway.ReconciliationReport,
) error {
	...
	for _, r := range report.Results {
		results = append(results, store.GatewayReconciliationResult{
	...
	stored := store.GatewayReconciliationReport{
		GatewayID:     gatewayID,
		Epoch:         epoch,
		Revision:      report.Revision,
		KeystoreState: report.KeystoreState,
		KeystoreBytes: report.KeystoreBytes,
		CheckedAt:     report.CheckedAt,
		LocalDevices:  report.LocalDevices,
		Results:       results,
	}
	err := s.reconciliation.Persist(ctx, stored, s.clock().UnixMilli())
	if err != nil {
...
func (s gatewayControlStore) IngestEvents(
	ctx context.Context,
	gatewayID string,
	epoch uint64,
	events []apigateway.GatewayEvent,
) error {
...
func (s gatewayControlStore) Lifecycle(
	...
	gatewayID string,
	epoch uint64,
	report apigateway.LifecycleReport,
) error {
...
func newPrivateGatewayGRPCServer(
	tlsConfig *tls.Config,
	auth privateGatewayAuthenticator,
	enrollment enrollmentRedeemer,
	renewal renewalIssuer,
	readiness func() error,
	control gatewayv1.GatewayControlServiceServer,
) *grpc.Server {
