...
func TestGatewayRepo_UpsertAndGet(t *testing.T) {
	...
	if got.ReconciliationStatus != "healthy" || got.KeystorePresent == nil || !*got.KeystorePresent || got.KeystoreBytes == nil || *got.KeystoreBytes != 12 {
		t.Fatalf("missing reconciliation metadata: %+v", got)
	}
