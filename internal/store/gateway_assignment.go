...
func (r *GatewayAssignmentRepo) Reassign(
	ctx context.Context,
	sessionID, newGatewayID string,
	at int64,
) (uint64, error) {
	...
	if err := tx.QueryRowContext(ctx,
		`SELECT gateway_id, assignment_epoch FROM gateway_session_assignments WHERE session_id=? FOR UPDATE`,
		sessionID,
	).Scan(&oldGatewayID, &epoch); err != nil {
	...
	var eligible int
	if err := tx.QueryRowContext(ctx,
		`SELECT COUNT(*) FROM gateways WHERE id=? AND deleted_at IS NULL AND status NOT IN ('pending_enrollment','disabled')`,
		newGatewayID,
	).Scan(&eligible); err != nil || eligible != 1 {
		return 0, fmt.Errorf("store: target gateway is not eligible")
	}
	result, err := tx.ExecContext(ctx,
		`UPDATE gateway_session_assignments SET gateway_id=?, assignment_epoch=assignment_epoch+1, updated_at=? WHERE session_id=?`,
		newGatewayID, at, sessionID,
	)
	if err != nil {
	...
	if _, err := tx.ExecContext(ctx,
		`UPDATE wa_sessions SET gateway_id=?, updated_at=? WHERE id=?`,
		newGatewayID, at, sessionID,
	); err != nil {
		return 0, fmt.Errorf("store: update session gateway pin: %w", err)
	}
	result, err = tx.ExecContext(ctx,
		`UPDATE gateways SET desired_revision=desired_revision+1, updated_at=? WHERE id IN (?, ?) AND deleted_at IS NULL`,
		at, oldGatewayID, newGatewayID,
	)
	if err != nil {
...
func (r *GatewayAssignmentRepo) UpdateConfig(
	ctx context.Context,
	sessionID string,
	autoRead, presenceTyping bool,
	ratePerMin, ratePerHour int32,
	at int64,
) error {
	...
	if err := tx.QueryRowContext(ctx,
		`SELECT gateway_id FROM gateway_session_assignments WHERE session_id=? FOR UPDATE`,
		sessionID,
	...
	if _, err := tx.ExecContext(ctx,
		`UPDATE wa_sessions SET auto_read=?, presence_typing=?, rate_per_min=?, rate_per_hour=?, updated_at=? WHERE id=?`,
		autoRead, presenceTyping, ratePerMin, ratePerHour, at, sessionID,
	); err != nil {
...
func assignSession(ctx context.Context, tx *sql.Tx, sessionID, gatewayID string, at int64) (uint64, error) {
	...
	result, err := tx.ExecContext(ctx,
		`INSERT INTO gateway_session_assignments (session_id, gateway_id, assignment_epoch, created_at, updated_at) VALUES (?, ?, 1, ?, ?)`,
		sessionID, gatewayID, at, at,
	)
	if err != nil {
...
func (r *GatewayAssignmentRepo) Unassign(ctx context.Context, sessionID string, at int64) error {
	...
	result, err := tx.ExecContext(ctx, `DELETE FROM gateway_session_assignments WHERE session_id=?`, sessionID)
	if err != nil {
...
func (r *GatewayAssignmentRepo) SetSessionDesired(ctx context.Context, sessionID string, run bool, at int64) error {
	...
	if _, err := tx.ExecContext(ctx,
		`UPDATE wa_sessions SET status=?, updated_at=? WHERE id=?`,
		status, at, sessionID,
	); err != nil {
