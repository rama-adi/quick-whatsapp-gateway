...
func TestGatewayRepoAllocateConnectionUsesCASAndRetries(t *testing.T) {
	...
	mock.ExpectExec("UPDATE gateways.*connection_epoch = connection_epoch \\+ 1").
		WithArgs(int64(100), int64(100), nil, nil, sqlmock.AnyArg(), 0, int64(0), domain.GatewayJoining, int64(100), "gw_1", uint64(4)).
		WillReturnResult(sqlmock.NewResult(0, 0))
	mock.ExpectQuery("SELECT connection_epoch.*FROM gateways").
		WithArgs("gw_1").
		WillReturnRows(sqlmock.NewRows([]string{"connection_epoch"}).AddRow(uint64(5)))
	mock.ExpectExec("UPDATE gateways.*connection_epoch = connection_epoch \\+ 1").
		WithArgs(int64(100), int64(100), nil, nil, sqlmock.AnyArg(), 0, int64(0), domain.GatewayJoining, int64(100), "gw_1", uint64(5)).
		WillReturnResult(sqlmock.NewResult(0, 1))
	mock.ExpectQuery("SELECT status.*FROM gateways").
		WithArgs("gw_1", uint64(6)).
		WillReturnRows(sqlmock.NewRows([]string{"status"}).AddRow(domain.GatewayJoining))

	accepted, err := repo.AcceptConnection(context.Background(), domain.GatewayConnectionHello{GatewayID: "gw_1", Status: domain.GatewayJoining}, 100)
	if err != nil || accepted.ConnectionEpoch != 6 || accepted.Status != domain.GatewayJoining {
...
func TestGatewayRepoAcceptReturnsPreservedAdministrativeDrain(t *testing.T) {
	...
	mock.ExpectExec("UPDATE gateways.*connection_epoch = connection_epoch \\+ 1").
		WithArgs(int64(100), int64(100), nil, nil, sqlmock.AnyArg(), 2, int64(0), domain.GatewayActive, int64(100), "gw_1", uint64(8)).
		WillReturnResult(sqlmock.NewResult(0, 1))
	mock.ExpectQuery("SELECT status.*FROM gateways").
		WithArgs("gw_1", uint64(9)).
		WillReturnRows(sqlmock.NewRows([]string{"status"}).AddRow(domain.GatewayDraining))
	...
	if err != nil || accepted.ConnectionEpoch != 9 || accepted.Status != domain.GatewayDraining {
		t.Fatalf("AcceptConnection: accepted=%+v err=%v", accepted, err)
	}
	if err := mock.ExpectationsWereMet(); err != nil {
		t.Fatal(err)
...
func TestGatewayRepoFencedWritesReturnFalseForStaleEpoch(t *testing.T) {
	...
	mock.ExpectExec("UPDATE gateways.*last_seen_at = \\?, session_count = \\?,\\s+status = CASE").
		WithArgs(int64(200), 3, int64(0), domain.GatewayDegraded, int64(200), "gw_1", uint64(9)).
		WillReturnResult(sqlmock.NewResult(0, 0))
	...
	mock.ExpectExec("UPDATE gateways.*status = CASE").
		WithArgs(int64(0), domain.GatewayDraining, int64(201), "gw_1", uint64(9)).
		WillReturnResult(sqlmock.NewResult(0, 1))
...
func TestGatewayRepoUnchangedCurrentEpochIsApplied(t *testing.T) {
	...
	mock.ExpectExec("UPDATE gateways.*status = CASE").
		WithArgs(int64(0), domain.GatewayActive, int64(200), "gw_1", uint64(9)).
		WillReturnResult(sqlmock.NewResult(0, 0))
	mock.ExpectQuery("SELECT EXISTS").
		WithArgs("gw_1", uint64(9)).
...
func TestGatewayRepoRuntimeReadyCannotUndoAdministrativeDrain(t *testing.T) {
	db, mock := newMock(t)
	repo := NewGatewayRepo(db)
	connection := domain.GatewayConnection{GatewayID: "gw_1", ConnectionEpoch: 9}

	mock.ExpectExec("UPDATE gateways.*status = CASE").
		WithArgs(int64(201), 3, int64(0), domain.GatewayActive, int64(201), "gw_1", uint64(9)).
		WillReturnResult(sqlmock.NewResult(0, 1))
	applied, err := repo.HeartbeatForEpoch(context.Background(), domain.GatewayHeartbeat{
	...
	querySource, err := os.ReadFile("queries/gateway.sql")
	if err != nil {
		t.Fatal(err)
	}
	sqlText := string(querySource)
	for _, guard := range []string{
		...
		if !strings.Contains(sqlText, guard) {
			t.Fatalf("gateway runtime status guard missing %q", guard)
		}
	}
}
