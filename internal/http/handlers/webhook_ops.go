...
type webhookBody struct {
	...
	CustomHeaders map[string]string   `json:"customHeaders,omitempty" doc:"Extra HTTP headers attached to every delivery POST. Use for static auth tokens or routing hints the receiver requires. Integrity headers (Content-Type and X-Webhook-*) are gateway-controlled and cannot be overridden." example:"{\"X-Tenant\":\"acme\"}"`
...
func RegisterWebhookOps(api huma.API, h *Handlers) {
	...
	huma.Register(api, huma.Operation{
		...
		Description: "Register a webhook endpoint for the caller's organization. Requires the `manage` capability.\n\n" +
			...
			"**Delivery headers.** Every `POST` also carries `X-Webhook-Request-Id` (the event id — use it to drop duplicate redeliveries) and `X-Webhook-Timestamp` (epoch milliseconds). Custom headers may add receiver-specific metadata, but cannot override gateway-controlled `Content-Type` or `X-Webhook-*` integrity headers.\n\n" +
