...
// TestContract_APIKeyHash hashes a fixed raw key with the gateway's production
// DefaultHasher and compares it with a digest generated by better-auth 1.6.22.
// The expected value is a hard-coded cross-language vector rather than a digest
// derived by the code under test. This pins the lookup half of the API-key trust
// seam so an upstream hashing change fails loudly instead of rejecting every
// valid key or encouraging an unsafe fallback.
...
// TestContract_APIKeyPermissions decodes representative better-auth permission
// maps with the same resource/action folding used by the production store. It
// expects only documented actions in the gateway bucket to grant typed
// capabilities. This pins a cross-language authorization contract so dependency
// drift cannot silently broaden an API key's access.
...
// TestContract_JWTShape compares the claim and role constants consumed by the Go
// verifier with the names emitted by better-auth's configured JWT payload. The
// cryptographic round trip is covered in jwt_test.go; this test isolates schema
// naming drift. A mismatch would otherwise authenticate a token but resolve the
// wrong organization or authority.
